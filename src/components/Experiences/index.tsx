...
export default function Experiences({ experiences }: ExperiencesProps) {
  return (
    <div className='py-8'>
      <Typography className='mb-8' variant='h3' bold>
        EXPERIENCES
      </Typography>
        ...
        {experiences.map((experience) => {
          return (
            <WithAnimateOnScroll amount={0.2} key={experience.title}>
              <div
                className='flex flex-col lg:flex-row gap-2 sm:gap-2 md:gap-4 lg:gap-0'
                key={experience.title}
              >
                <div className='w-full lg:w-1/4'>
                  <Typography variant='h6' italic>
                    {experience.experienceDetail.startDate} - {experience.experienceDetail.endDate}
                  </Typography>
                  <Tag variant='outline'>
                ...
                <div className='w-full lg:w-3/4 rounded-md border border-solid border-black dark:border-white p-4'>
                  <Typography variant='h6'>
                    {experience.experienceDetail.companyUrl && (
                      <Link
                        className='font-bold hover:underline p-2'
                        to={experience.experienceDetail.companyUrl}
                        target='_blank'
                      >
                    ...
                    {experience.experienceDetail.roleTitle}
                  </Typography>
                  <div
