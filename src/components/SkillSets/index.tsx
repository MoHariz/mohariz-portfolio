      ...
      <WithAnimateOnScroll amount={0.2}>
        <div className='grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4'>
          {skillSets.map((skillSet) => {
            return (
              <div
                className='border rounded-md border-solid border-black dark:border-white p-4'
                key={skillSet.title}
              >
                <Typography variant='h5' className='mb-4' bold>
                  {skillSet.title}
                </Typography>
                <div className='flex flex-wrap gap-2'>
                  {skillSet.skills.nodes.map((skill) => {
                    return <Tag key={skill.name}>{skill.name}</Tag>;
                  })}
                </div>
              </div>
            );
